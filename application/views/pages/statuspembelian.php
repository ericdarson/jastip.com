		...
		echo "<div class=\"ui stacked segment\">
				...
				<table class='table table-striped table-bordered display padding' width='100%' cellpadding=\"10\" border=\"1\">
					...
					<tbody>";
							...
							if($row->status==9 ||$row->status==7 || $row->status==10){
								...
								if($row->status==1){
									...
									echo '<td align ="center" width="25%"><h3>Barang Sedang dikirim oleh Traveler</h3>';
									echo '<input type="submit" name="btnBayar" class="ui teal button" formaction ="'. base_url().'index.php/KonfirmasiController/status_terima_barang" value="Konfirmasi Terima Barang"/></td>';
								}
								else if($row->status==6){
									echo '<td align ="center" width="25%"><h3>Transaksi Selesai</h3>';
									echo '<input type="submit" name="btnBayar" class="ui teal button" formaction ="'. base_url().'index.php/KonfirmasiController/status_ubah_pembelian7" value="Hapus Transaksi"/></td>';
								}
								else if($row->status==8){
									echo '<td align ="center" width="25%"><h3>Transaksi Selesai</h3>';
									echo '<input type="submit" name="btnBayar" class="ui teal button" formaction ="'. base_url().'index.php/KonfirmasiController/delete_permanen" value="Hapus Transaksi"/></td>';
								}
								else if($row->status==99){
									echo '<td align ="center" width="25%"><h3>Maaf Traveler Sedang Tidak Bisa Menerima Pesanan Kamu</h3>';
									echo '<input type="submit" name="btnBayar" class="ui teal button" formaction ="'. base_url().'index.php/KonfirmasiController/status_ubah_pembelian10" value="Hapus Transaksi"/></td>';
								}
								else if($row->status==11){
									echo '<td align ="center" width="25%"><h3>Barang telah ditolak</h3>
									<input type="submit" name="btnKirim" class="ui teal button" formaction ="'. base_url().'index.php/KonfirmasiController/delete_permanen" value="Hapus"/>
									</td>';
								}
								echo '</form>';
								echo "<tr>";
